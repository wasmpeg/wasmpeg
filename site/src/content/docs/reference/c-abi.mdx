...
on success and a negative [`AVERROR`](/reference/errors/) on failure. Accessor functions return
...
See [error codes](/reference/errors/) for the negative return values. To add a new exported
function, see [configuration](/build/configuration/).
