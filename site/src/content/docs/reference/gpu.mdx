...
Every function wraps the [C ABI](/reference/c-abi/) via `ccall` and handles the heap copies for
...
shared session slots ([8 for decoders/audio/probe, 4 for encoders](/reference/c-abi/)). The
...
exported. See [WebGPU](/webgpu/) for how to surface these in a UI.
