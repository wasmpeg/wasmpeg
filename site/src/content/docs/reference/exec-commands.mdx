...
- [`wasmpeg.run(input, args)`](/reference/high-level/#wasmpegruninput-args) — high-level, accepts any input type.
- [`ff.exec(args)`](/reference/ffmpeg-class/#ffexecargs-opts) — the `FFmpeg` class; reads `-i` from the virtual filesystem.
...
| Decode | input only, no filter | a [Decoder](/reference/high-level/#decoder-object) |
| Audio | `-af` or `-vn` | an [AudioDecoder](/reference/high-level/#audiodecoder-object) |
...
pass a demuxer hint. Use [`wasmpeg.decode(input, { format })`](/guides/decode-video/#forcing-a-demuxer)
for those.
...
stream. To filter every frame, open a [decoder](/guides/decode-video/) and scale per
frame, or push frames through [`gpu.createEncoder`](/reference/gpu/#gpucreateencoderopts).
</Aside>
...
this is only needed to force it. See [WebGPU](/webgpu/).
...
decoder and returns an [AudioDecoder](/reference/high-level/#audiodecoder-object).
...
passed on this path. Filter audio with a [filtergraph through the decoder](/guides/decode-audio/)
...
[`gpu.createEncoder`](/reference/gpu/#gpucreateencoderopts).
...
high-level [`encode({ frames })`](/reference/high-level/#wasmpegencodeinput-opts).
...
[`wasmpeg.decode(input, { format })`](/guides/decode-video/#forcing-a-demuxer), which
