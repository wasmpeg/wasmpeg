...
- **[wasmpeg](/reference/high-level/)** — the default export. Pass a `File`, `Blob`, URL,
  `Uint8Array`, or canvas; it handles the rest.
- **[FFmpeg](/reference/ffmpeg-class/)** — a familiar `load`/`writeFile`/`exec` surface with
  a virtual filesystem, for porting ffmpeg.wasm code.
- **[gpu](/reference/gpu/)** — the typed pipeline; you pass raw bytes/RGBA and manage
...
- **WebGPU build** (`webgpu.wasm`) — adds a `scale_webgpu` filter that runs the resize on
  the GPU. Experimental; see [WebGPU](/webgpu/).
...
exhaust the pool and the next open throws [`ENOMEM`](/reference/errors/).
</Aside>
...
[command reference](/reference/exec-commands/) for exactly what the dispatcher supports.
...
- [Installation](/start/installation/) — get it into your project.
- [The three APIs](/start/apis/) — pick the right level.
- [Quick start](/start/quick-start/) — decode, scale, probe, encode in a few lines.
