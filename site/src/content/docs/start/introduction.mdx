...
from a 2.9 MB gzipped WASM, with **no SharedArrayBuffer and no COOP/COEP headers**
required.
...
For migration, the [`FFmpeg` compat class](/start/apis/) mirrors enough of the
...
<LinkCard title="Installation" href="/start/installation/" description="npm, CDN, Node, and bundler setup." />
<LinkCard title="Quick start" href="/start/quick-start/" description="Decode, scale, probe, and encode in a few lines." />
<LinkCard title="The three APIs" href="/start/apis/" description="High-level, compat, and low-level — pick the right one." />
<LinkCard title="How it works" href="/start/how-it-works/" description="The layers, the pipeline, and the session model." />
