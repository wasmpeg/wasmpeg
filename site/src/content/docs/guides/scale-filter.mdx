...
2. Scale to a target. The default graph is a plain resize.

   ```js
   const rgba = await wasmpeg.scale(file, 1280, 720);   // RGBA8 at 1280×720
   ```

3. Drop the result onto a canvas.
...
[configuration reference](/build/configuration/) lists the full set, and the
[filters page](/filters/) documents what each one does.
...
[`gpu.benchGpu` / `gpu.benchCpu`](/reference/gpu/).
</Aside>
...
[decoder](/guides/decode-video/) and call `nextFrame(w, h)` in a loop, which also scales on
decode, or run frames through [`encode()`](/guides/encode/) to get encoded image output.

## See also

- [Decode video frames](/guides/decode-video/) — process every frame, not just the first.
- [Encode & thumbnails](/guides/encode/) — turn a filtered frame into a JPEG or PNG.
- [GPU reference](/reference/gpu/) — the lower-level `gpu.scale` and the benchmarks.
