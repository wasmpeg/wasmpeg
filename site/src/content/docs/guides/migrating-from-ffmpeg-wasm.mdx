...
wasmpeg ships an [`FFmpeg` class](/reference/ffmpeg-class/) shaped like `@ffmpeg/ffmpeg`
...
- **Smaller.** A focused decode-first build (~2.9 MB gzipped CPU).
...
value. A decode-only command returns a [Decoder](/reference/high-level/#decoder-object); a
...
[high-level API](/reference/high-level/), which is purpose-built for these tasks and accepts
...
- [FFmpeg class reference](/reference/ffmpeg-class/) · [Command reference](/reference/exec-commands/)
- [The three APIs](/start/apis/) — the high-level export is usually the better target.
