...
→ [Scale & filter guide](/guides/scale-filter/)
...
[geometry filters](/reference/exec-commands/#geometry-filters).
</Aside>
...
Any filter compiled into the build works — see the [filter reference](/filters/) for the
...
→ [Decode video guide](/guides/decode-video/)
...
chunk-to-`AudioBuffer` routine, see the [Examples](/guides/examples/) page.

→ [Decode audio guide](/guides/decode-audio/)
...
[`gpu.createEncoder`](/reference/gpu/#createencoder) yourself. For full
...
[exec() command support](/reference/exec-commands/) page lists exactly which raw arg
