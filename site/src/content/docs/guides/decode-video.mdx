...
that's a thrown error pointing you at [`scale()`](/guides/scale-filter/) instead.
...
1. Load the module once, at startup.
...
2. Open a decoder and read its dimensions.
...
3. Pull frames until the stream ends, then close.
...
[`probe()`](/guides/probe/), which reports `fpsNum` and `fpsDen` separately.
...
See the [error reference](/reference/errors/) for what the negative return codes mean.

## See also

- [Scale & filter a frame](/guides/scale-filter/) — one-shot frame processing without
  managing a decoder.
- [Probe metadata](/guides/probe/) — read dimensions, duration, and stream layout before
  you decide to decode.
- [Decode audio to PCM](/guides/decode-audio/) — the matching iterator for audio streams.
