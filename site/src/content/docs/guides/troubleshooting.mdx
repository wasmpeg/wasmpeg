...
16.4+. Check the [support matrix](/start/support/). There's no non-SIMD fallback in the
default build.
...
encode are identical on both. See [WebGPU](/webgpu/).
...
You're leaking [session slots](/start/how-it-works/#sessions-and-memory). There are only 8
...
- If the **codec isn't in your build**, you'll get
  [`AVERROR_DECODER_NOT_FOUND`](/reference/errors/). The default build is broad; check the
  [codec list](/codecs/) and, for custom builds, add it to a preset and rebuild.
...
See [Configuration](/build/configuration/).
...
[`gpu.createEncoder`](/reference/gpu/#createencoder) for image output. See the
[command reference](/reference/exec-commands/).
...
[accepted but ignored options](/reference/exec-commands/#accepted-but-ignored-options).

### My audio filter isn't applied

The dispatcher's audio path returns decoded PCM but does **not** apply `-af` graphs. Filter
audio in the Web Audio graph after [decoding to PCM](/guides/decode-audio/) instead.
...
`transpose=1,scale=720:1280`. See [geometry filters](/reference/exec-commands/#geometry-filters).
...
host with no special headers. See [support](/start/support/#no-cross-origin-isolation-needed).
...
[Node guide](/guides/frameworks/nodejs/).
...
[`gpu.createEncoder`](/reference/gpu/#createencoder) directly and `pushRgba` your frames.
