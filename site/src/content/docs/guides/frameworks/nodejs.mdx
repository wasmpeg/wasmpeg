...
1. The project must be ESM, since wasmpeg ships ESM only. Set `"type": "module"` in
...
2. Probe and decode a file. Read it off disk into a `Uint8Array` and hand that to wasmpeg:
...
- [Probe metadata](/guides/probe/) · [Decode audio](/guides/decode-audio/) ·
  [Encode & thumbnails](/guides/encode/) · [High-level reference](/reference/high-level/)
