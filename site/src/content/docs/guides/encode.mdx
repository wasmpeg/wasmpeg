...
1. Load and encode the first frame as JPEG.
...
2. Turn the bytes into something usable.
...
`wasmpeg-full`. The [codec table](/codecs/) has the complete list and which build each
encoder belongs to.
...
- [Scale & filter a frame](/guides/scale-filter/) — filter a frame before encoding it.
- [Decode video frames](/guides/decode-video/) — pull frames yourself for finer control.
- [Codec & format support](/codecs/) — the full encoder list per build.
