...
particular binary actually contains, see [Configuration & presets](/build/configuration/).
...
to the [compatibility report](/compatibility/) if you care about bit-exact results. A
...
[Configuration](/build/configuration/#the-image-pipe-naming-trap).
</Aside>
...
<Aside type="note">
Muxers are used by the [encoder](/reference/gpu/#createencoder) for image and audio
...
right pixels." See the **[live Compatibility page](/compatibility/)** for the current
...
[`decode(input, { format })`](/guides/decode-video/#forcing-a-demuxer). If a codec is
genuinely not in your build, decoding fails with
[`AVERROR_DECODER_NOT_FOUND`](/reference/errors/); add it to a preset and rebuild per
[Configuration](/build/configuration/#add-a-preset).
